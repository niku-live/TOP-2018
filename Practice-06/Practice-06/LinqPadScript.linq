...
void Main()
{
	...
	var list4 = (
	  ...
	  where item % 2 == 0
	  orderby item
	  select new { Number = item, Text = item.ToString() + "%", A=5 });
	list4.Dump();
   ...
   //list.Where(l < 7).ToList().Where(l => f(L)).Where(l => l < 7).ToList();
   //list.Where(l => l > 5).Where(l => l < 7).ToList();
}